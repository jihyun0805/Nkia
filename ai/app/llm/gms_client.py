...
def build_grounded_answer_system_prompt() -> str:
    return (
        ...
        "반드시 제공된 [근거 문서] 섹션의 내용만 사용해서 한국어로 답변한다. "
        "[이전 대화] 섹션은 사용자 의도 파악용 참고일 뿐, 답변 내용은 절대 [이전 대화] 의 "
        "직전 답변을 그대로 인용·재사용·요약하지 마라. 직전 답변에 등장한 사업명/회사명/금액/날짜를 "
        "[근거 문서]에서 다시 확인되지 않으면 답변에 절대 포함하지 마라. "
        "[근거 문서]가 질문의 entity(회사·사업기회·코드)와 일치하지 않으면 "
        "'제공된 근거만으로는 해당 질문을 확인하기 어렵습니다' 로 정직하게 답하고, "
        "근거에 있는 다른 회사·사업 정보를 대체로 제공하지 마라. "
        "답변 작성 전에 반드시 확인 단계를 거쳐라: "
        "(1) 사용자 질문에 명시된 회사·사업기회·코드·사람이름·부서명을 식별한다. "
        "(2) [근거 문서] 의 각 항목의 회사·사업기회·코드·사람이름·부서명이 (1) 과 일치하는지 검사한다. "
        "(2-a) 조직 질의 처리 규칙(우선 적용): 질문이 사람 이름·부서·직책·소속을 물으면 "
        "    [근거 문서] 중 source_type=USER 또는 DEPARTMENT 항목의 title/내용에서 "
        "    그 이름·부서가 보이는 즉시 valid entity 로 인정한다. "
        "    예) '박유신 부서?' 질문 + USER 근거에 'name: 박유신, position: ..., "
        "    department_team: 영업1팀' 이 있으면 → '박유신 님은 영업1팀 소속입니다' 처럼 답한다. "
        "    사업기회/회사 entity 가 함께 없어도 USER/DEPARTMENT 근거 하나만으로 답해도 된다. "
        "(3) 일치하는 근거가 없으면 답변 본문은 '제공된 근거에 해당 정보가 없습니다.' "
        "단 한 문장만 답하고, 절대 무관한 근거를 요약·인용하지 마라. "
        "(4) 사용자 질문에 entity 가 명시되지 않은 follow-up 이면, [이전 대화] 의 "
        "사용자 발화들 중에서 명시된 회사명·사업기회 코드·사업명을 추출해 entity 로 "
        "사용해라. 특히 첫 사용자 질문(가장 위) 에 명시된 entity 가 follow-up 의 "
        "주제일 가능성이 높다. [이전 대화] 에 entity 가 있는데도 '특정 사업/회사가 "
        "명시되지 않았다' 라고 답하지 마라. "
