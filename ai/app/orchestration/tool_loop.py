# 인수인계: 도구 호출형 실행 루프입니다. 계획된 tool spec을 executor에 넘기고 결과를 모아 후속 답변에 사용할 형태로 정리합니다.
# 핵심 흐름: LangGraph 본선에서 바로 답하지 못하는 도메인 작업을 확장할 때 연결 지점이 됩니다.
# 같이 확인: 새 tool은 tools/specs.py에 정의하고 tools/executor.py 실행 결과 계약을 같이 맞추세요.
