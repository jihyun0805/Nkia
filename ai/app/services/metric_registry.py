# 인수인계: 정형 질의에서 쓰는 매출, 계약금액, 위험도 같은 metric key와 표시 라벨/정렬 기준을 관리합니다.
# 핵심 흐름: 사용자 질문의 “큰/높은/낮은” 표현이 어떤 DB 필드로 매핑되는지 추적할 때 확인합니다.
# 같이 확인: 새 지표는 query_intent_service.py와 structured_answer_service.py의 응답 생성도 같이 확장하세요.
