# 인수인계: 명함 이미지 OCR 실행과 후처리 파이프라인입니다. PaddleOCR 결과를 줄 단위 텍스트로 만들고 필드 분류기에 넘깁니다.
# 핵심 흐름: 이미지 품질/회전/언어 설정에 민감하므로 OCR 실패는 extractor보다 여기 로그를 먼저 확인합니다.
# 같이 확인: API 응답 구조는 schemas/ocr.py와 api/ocr.py가 소비합니다.
