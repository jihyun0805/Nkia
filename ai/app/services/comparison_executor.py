# 인수인계: 두 사업/문서의 비교 질문을 정형 데이터와 근거 요약으로 계산하는 executor입니다.
# 핵심 흐름: LangGraph comparison 노드가 이 파일을 호출하고, 결과는 format_comparison_answer로 바로 사용자 답변이 됩니다.
# 같이 확인: 비교 지표 추가 시 langgraph/comparison.py의 metric 추론과 함께 맞추세요.
