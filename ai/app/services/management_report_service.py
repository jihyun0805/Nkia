# 인수인계: 기간/주제별 검색 근거를 모아 경영 리포트 형태의 답변을 구성합니다.
# 핵심 흐름: 일반 챗봇보다 긴 요약과 지표형 문장을 만들기 때문에 confidence와 근거 제한을 별도로 봅니다.
# 같이 확인: 리포트 프롬프트나 출력 필드 변경 시 schemas/report.py와 api/report.py를 같이 확인하세요.
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "- 마크다운 형식으로 작성한다.",
            "- 전체 답변을 ```markdown 또는 ``` 코드블록으로 감싸지 않는다.",
            "- 아래 '출력 형식'의 제목과 순서를 그대로 사용한다.",
            "- '왜냐하면' 같은 구어체 연결 문장을 쓰지 않는다.",
            "- 요약과 종합 의견은 보고서 문체의 짧은 문단으로 작성한다.",
            "- `## 요약` 섹션에는 표를 쓰지 않는다. `## 요약` 다음에는 반드시 일반 문장 3~5개만 작성하고, 파이프 문자(|)나 표 구분선(|---|---|)을 절대 쓰지 않는다.",
            "- 본문 문장은 모두 `~다` 체로 통일한다.",
            "- `합니다`, `됩니다`, `해주세요`, `입니다`, `습니다`, `요`로 끝나는 문장을 쓰지 않는다.",
            "- 현황, 리스크, 액션은 마크다운 표를 우선 사용한다.",
            "- 표는 반드시 헤더 행, 구분 행, 데이터 행을 모두 포함한 표준 마크다운 테이블로 작성한다. 구분 행(|---|---|)만 단독으로 출력하지 않는다.",
