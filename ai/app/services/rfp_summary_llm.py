# 인수인계: RFP 본문을 LLM에 보내 짧은 요약을 만드는 프롬프트/호출 래퍼입니다.
# 핵심 흐름: 요약은 저장/색인하지 않고 API 응답으로만 반환되므로 근거 기반 챗봇 답변과 분리되어 있습니다.
# 같이 확인: 문체나 요약 항목 변경은 schemas/rfp_summary.py 응답 필드와 같이 조정하세요.
