...
def normalize_resolution_terms(query_terms: list[str]) -> list[str]:
    ...
    ignored = {
        ...
        "관련", "관련된", "포함된",
        # follow-up 일반 명사 — entity 없는 짧은 follow-up 이 사업명 부분 매칭으로
        # 잘못된 사업 lock 되는 것 방지 (예: "진행 단계 알려줘" → "...코어뱅킹 2단계..." 매칭)
        "단계", "진행", "이력", "내용", "정보", "확인", "요청", "다음",
        "최근", "이번", "지난", "어디",
        # 사업명에 흔히 등장하는 generic 명사 — entity 토큰 매칭 noise 차단
        "운영", "관리", "도입", "사업기회", "프로젝트",
        "ITSM",
        "EMS",
        "AIOPS",
