# 인수인계: 답변에 create_draft/edit_field/navigate 액션을 붙이는 LangGraph 후처리입니다.
# 핵심 흐름: 사용자 발화와 근거를 보고 프론트가 누를 수 있는 액션 버튼 payload를 만듭니다.
# 같이 확인: 프론트 ChatActions와 use-chatbot-prefill의 payload 계약을 같이 확인하세요.
