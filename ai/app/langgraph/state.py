# 인수인계: LangGraph 전체가 공유하는 GraphState와 하위 상태 모델입니다.
# 핵심 흐름: preflight부터 answer까지 모든 노드가 이 모델을 직렬화해서 주고받습니다.
# 같이 확인: 필드 추가는 runtime state, adapter, 테스트 fixture를 함께 고쳐야 합니다.
