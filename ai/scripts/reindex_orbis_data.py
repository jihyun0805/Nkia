# 인수인계: 운영/로컬 백엔드 DB 전체를 읽어 AI 색인을 end-to-end로 재생성하는 배치 스크립트입니다.
# 핵심 흐름: pg_notify 누락, seed 재주입, 임베딩 모델 변경 후에는 이 스크립트로 source/chunk를 다시 밀어 넣습니다.
# 같이 확인: 테이블별 build_current_*_documents 함수와 DOCUMENT_CONFIGS가 실제 색인 문서 모양을 결정합니다.
#!/usr/bin/env python3
