...
export default function ActivityPage() {
  ...
  const activeQuotationRecords = useMemo(() => quotationRecords.filter((item) => !item.deletedAt), [quotationRecords]);
  ...
  }).filter((item) => matchesSearch([item.id, item.customerCode, item.customer, item.opportunity, item.activityMode, item.activityContent, item.location, item.attendees]));
  ...
  }).filter((item) => matchesSearch([item.id, item.customer, item.opportunity, item.items.map((entry) => entry.name).join(", "), item.amount, item.validity, getQuotationDisplayStatus(item)]));
  ...
  const filteredRequests = filterRecords(activityRequests, filters, {
    ...
    .filter((item) => matchesSearch([item.id, item.type, item.requester, item.receiver, item.customer, item.opportunity, item.status, item.date, item.dueDate]))
    .sort((a, b) => b.date.localeCompare(a.date));

  const completedActivityRequests = useMemo(() => activityRequests.filter((item) => item.status === "접수완료").sort((a, b) => a.dueDate.localeCompare(b.dueDate)), [activityRequests]);
  ...
  const monthlyCompletedRequests = useMemo(
    () =>
      completedActivityRequests.filter((item) => {
        ...
        return activityDate.getFullYear() === month.getFullYear() && activityDate.getMonth() === month.getMonth();
      }),
  ...
  const completedRequestDates = useMemo(() => completedActivityRequests.map((item) => new Date(`${item.dueDate}T00:00:00`)), [completedActivityRequests]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col">
        <Header title="활동" description="영업활동 내역, 견적, 활동 요청을 통합 관리합니다" />
              ...
              <div className="flex items-center gap-2">
                ...
                {activeTab === "quotations" && (
                  <Button asChild className="bg-primary hover:bg-primary/90">
                    <Link href="/admin?tab=products">표준가격표</Link>
                  </Button>
                )}
              </div>
                ...
                {activeTab === "quotations" ? (
                  <>
                    <Button asChild className="gap-2 bg-primary hover:bg-primary/90">
                      <Link href="/activity/new/quotations">
                        <Plus className="w-4 h-4" />
                        견적 등록
                      </Link>
                    </Button>
                    <Button asChild className="gap-2 bg-primary hover:bg-primary/90">
                      <Link href="/maintenance/quotation/new">
                        <Plus className="w-4 h-4" />
                        유지보수견적 등록
                      </Link>
                    </Button>
                  ...
                  <Button asChild className="bg-primary hover:bg-primary/90">
                    <Link href={`/activity/new/${activeTab}`}>
                      ...
                      {activeTab === "activities" ? "활동 등록" : "활동요청 등록"}
                    </Link>
                  </Button>
                )}
              </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    {activityCustomerCards.length > 0 ? (
                      ...
                      <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">조건에 맞는 고객사 활동이 없습니다.</div>
                    )}
                  </CardContent>
                </Card>
              </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredQuotations.map((quote) => (
                        <TableRow key={quote.id} className="cursor-pointer hover:bg-muted/50" onClick={() => router.push(`/activity/quotations/${quote.id}`)}>
                        ...
                        <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
                          <div className="overflow-x-auto">
                            <MonthCalendar
                              ...
                              modifiersClassNames={{
                                completedRequestDate: "bg-primary text-primary-foreground rounded-md hover:bg-primary hover:text-primary-foreground",
                          ...
                          <div className="space-y-3">
                            {monthlyCompletedRequests.map((request) => (
                              <button
                                ...
                                className={`w-full rounded-lg border p-4 text-left transition-colors ${
                                  selectedCalendarDate?.toISOString().slice(0, 10) === request.dueDate ? "border-primary bg-primary/5" : "hover:bg-muted/50"
                                }`}
                                onClick={() => {
                            ...
                            {monthlyCompletedRequests.length === 0 && (
                              <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">선택한 월에 접수완료된 영업 활동요청이 없습니다.</div>
                            )}
                          </div>
                        </div>
                      </CardContent>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredRequests.map((req) => (
                          <TableRow key={req.id} className="cursor-pointer hover:bg-muted/50" onClick={() => router.push(`/activity/requests/${req.id}`)}>
