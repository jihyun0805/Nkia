...
import { Search, ClipboardList, Receipt, Wallet, TrendingUp } from "lucide-react"
...
export default function ProjectPage() {
  ...
  const filteredProjectResults = filterRecords(projectResults, filters, { status: (item) => item.status, owner: (item) => item.pm, date: (item) => item.startDate, fields: { customer: (item) => item.customer } }).filter((item) => [item.id, item.contractId, item.name, item.customer, item.pm, item.issues].join(" ").toLowerCase().includes(q))
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6 overflow-auto">
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "results" | "billing" | "collection" | "revenue")} className="space-y-6">
            ...
            <TabsContent value="results">
              <Card><CardHeader className="pb-4"><div className="flex items-center justify-between"><CardTitle className="text-lg">사업 현황</CardTitle><Badge variant="secondary">{filteredProjectResults.length}건</Badge></div></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead className="w-[120px]">사업번호</TableHead><TableHead>계약번호</TableHead><TableHead>사업명</TableHead><TableHead>고객사</TableHead><TableHead>PM</TableHead><TableHead>사업기간</TableHead><TableHead>진행률</TableHead><TableHead>상태</TableHead><TableHead>이슈</TableHead></TableRow></TableHeader><TableBody>{filteredProjectResults.map((project) => <TableRow key={project.id} className="cursor-pointer hover:bg-muted/50" onClick={() => router.push(`/project/results/${project.id}`)}><TableCell className="font-mono text-sm">{project.id}</TableCell><TableCell className="font-mono text-sm">{project.contractId}</TableCell><TableCell className="font-medium max-w-[150px] truncate">{project.name}</TableCell><TableCell>{project.customer}</TableCell><TableCell>{project.pm}</TableCell><TableCell className="text-sm">{project.startDate} ~ {project.endDate}</TableCell><TableCell><div className="flex items-center gap-2"><div className="w-16 bg-muted rounded-full h-2"><div className={`h-2 rounded-full ${project.progress === 100 ? "bg-green-500" : "bg-primary"}`} style={{ width: `${project.progress}%` }} /></div><span className="text-sm">{project.progress}%</span></div></TableCell><TableCell><Badge variant={project.status === "완료" ? "default" : "secondary"} className={project.status === "완료" ? "bg-green-100 text-green-700 hover:bg-green-100" : "bg-blue-100 text-blue-700 hover:bg-blue-100"}>{project.status}</Badge></TableCell><TableCell className="max-w-[150px] truncate">{project.issues}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
            </TabsContent>
