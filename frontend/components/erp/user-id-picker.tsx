...
export function UserIdPicker({ value, users, onValueChange, placeholder = "사용자를 선택하세요", disabled = false, emptyMessage = "일치하는 사용자가 없습니다." }: UserIdPickerProps) {
  ...
  const suggestions = useMemo(() => {
    ...
    const hits = fuzzyMatch(trimmed, users, (user) => [user.name ?? "", user.employeeNumber ?? "", user.email ?? "", user.id ?? "", user.departmentName ?? ""], 8);
    ...
    <Popover open={open && !disabled} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[var(--radix-popover-anchor-width)] p-0" align="start" onOpenAutoFocus={(event) => event.preventDefault()}>
        <Command shouldFilter={false}>
          <CommandInput placeholder="이름, 부서, 사번, 이메일로 검색" value={query} onValueChange={setQuery} />
          <CommandList>
            ...
            <CommandGroup>
              {suggestions.map((user) => {
                ...
                return (
                  <CommandItem key={user.id ?? user.email ?? user.name} value={user.id ?? user.name ?? ""} onMouseDown={(event) => event.preventDefault()} onSelect={() => commitSelection(user)}>
