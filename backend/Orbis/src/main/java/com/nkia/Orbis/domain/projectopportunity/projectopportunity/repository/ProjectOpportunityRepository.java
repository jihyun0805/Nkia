...
public interface ProjectOpportunityRepository extends JpaRepository<ProjectOpportunity, Long> {
  ...
  /**
   * [1] 목록 조회 (페이징 + 정렬 지원) - @EntityGraph를 사용하여 N+1 문제를 방지합니다. - 목록 응답 DTO에서 고객사 이름이 필요하므로, DB에서
   * 조회할 때 Left OuterJoin으로 한 번에 가져옵니다.
   */
  @EntityGraph(attributePaths = {"customerCompany", "salesRepresentative"})
  Page<ProjectOpportunity> findAll(Pageable pageable);

  /**
   * [2] 단건 상세 조회 - 상세 조회 시에도 고객사 정보가 바로 필요하므로 Fetch Join(EntityGraph)을 적용합니다. - 기본 findById를
   * 오버라이딩하여 최적화합니다.
   */
  @Override
  @EntityGraph(attributePaths = {"customerCompany", "salesRepresentative"})
