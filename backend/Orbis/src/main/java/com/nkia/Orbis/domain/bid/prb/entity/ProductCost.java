...
public class ProductCost {
    ...
    public ProductCost copy() {
        ProductCost copy = new ProductCost();

        copy.items = this.items != null ?
                this.items.stream().map(ProductCostItem::copy).toList() : new ArrayList<>();

        copy.totalListPrice = this.totalListPrice;
        copy.totalProductCost = this.totalProductCost;
